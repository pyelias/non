...
pub trait StoresIn<HasSlot, Value> {
    fn get(has_slot: &HasSlot) -> Value where Value: Copy;
    ...
    #[inline]
    fn take(has_slot: &mut HasSlot) -> Value where Value: Default {
...
impl<HasSlot, Value> StoresIn<HasSlot, Value> for DefaultSlot where HasSlot: Stores<Value> {
    #[inline]
    fn get(has_slot: &HasSlot) -> Value where Value: Copy {
        has_slot.get()
    }
...
impl<Struct, Value, Field: GetsField<Struct>, Subslot: StoresIn<Field::FieldType, Value>> StoresIn<Struct, Value> for FieldSlot<Field, Subslot> {
    #[inline]
    fn get(has_field: &Struct) -> Value where Value: Copy {
    ...
    #[inline]
    fn take(has_field: &mut Struct) -> Value where Value: Default {
        let has_slot = Field::get_mut(has_field);
